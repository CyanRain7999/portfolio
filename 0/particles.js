...
(() => {
  ...
  // 重生/穿屏后短暂“冷却”：避免刚重生就和远处粒子连线，出现跨屏大三角/大线段
  const SPAWN_COOLDOWN = 18; // 约 18 帧（dt≈1/帧）
  const EDGE_CUTOFF = 0.48;  // 跨屏保护：|dx|>|w|*0.48 或 |dy|>|h|*0.48 时，禁止连线
  ...
  class Particle {
    ...
    update(dt, w, h, t, mouse) {
      if (this.spawnCooldown > 0) this.spawnCooldown = Math.max(0, this.spawnCooldown - dt);
      this.x += this.dx * dt;
      ...
      if (this.x < -20) { this.x = w + 20; this.spawnCooldown = SPAWN_COOLDOWN; }
      if (this.x > w + 20) { this.x = -20; this.spawnCooldown = SPAWN_COOLDOWN; }
    }
  ...
  function drawConnections() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        // 防跨屏：如果两点在屏幕上“看起来很远”，直接不连线（避免穿屏/重生瞬间的大三角/大线段）
        if (Math.abs(rawDx) > width * EDGE_CUTOFF || Math.abs(rawDy) > height * EDGE_CUTOFF) continue;
